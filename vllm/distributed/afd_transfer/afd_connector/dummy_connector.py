...
This connector provides a no-op implementation of the AFDConnectorBase
interface,
useful for testing and development scenarios where actual distributed FFN
computation
is not needed.
"""
import time
...
class DummyAFDConnector(AFDConnectorBase):
    ...
    def send_attn_output(
        ...
        logger.debug("DummyAFDConnector: send_attn_output layer=%s, stage=%s",
                     metadata.layer_idx, metadata.stage_idx)
    ...
    def recv_attn_output(
        ...
        """Receive attention output from attention workers (dummy 
        implementation)."""
    ...
    def send_ffn_output(
        ...
        logger.debug("DummyAFDConnector: send_ffn_output layer=%s, stage=%s",
                     metadata.layer_idx, metadata.stage_idx)

        # Validate that ffn_output shape matches metadata
        if not metadata.validate_tensor_shape(ffn_output.shape):
            logger.warning("FFN output shape %s doesn't match metadata %s",
                           ffn_output.shape, metadata)

        # Log the splitting information for debugging
        logger.debug(
            "DummyAFDConnector: Would split FFN output into %s parts "
            "with lengths %s", metadata.num_sequences, metadata.seq_lens)
        ...
        if metadata.get_split_indices():
            ...
            logger.debug("DummyAFDConnector: Split shapes: %s",
